...
var Collection = {
    ...
    getCurrentAccountUid: async function() {
        ...
        // 优先使用 genshin.uid() API 获取 UID：去除非数字字符得到纯数字 UID，更快更稳定
        try {
            const rawUid = await genshin.uid();
            const apiUid = rawUid ? String(rawUid).replace(/[^0-9]/g, '') : "";
            if (apiUid) {
                checkResult.accountUid = apiUid;
                log.info(`✅ 通过 genshin.uid() API 获取当前账号UID：${Utils.maskUid(checkResult.accountUid)}`);
                return checkResult.accountUid;
            }
            log.warn(`⚠️ genshin.uid() API 返回为空，回退到旧 OCR 识别`);
        } catch (e) { if (Utils.isCancellationError(e)) throw e;
            log.warn(`⚠️ genshin.uid() API 调用失败：${e.message}，回退到旧 OCR 识别`);
        }
    ...
    // 执行脚本
    executeScripts: async function(scriptList, startIndex, maxCount, currentUid, cooldown, cooldownRecord, onProgress) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            try {
                ...
                if (typeof onProgress === 'function') {
                    onProgress(script, i + 1, endIndex);
                }
