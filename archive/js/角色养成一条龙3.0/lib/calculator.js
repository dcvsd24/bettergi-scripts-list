...
// Planner-side target material calculations used by configGenerator.js.
var CultivationMaterialCalculator = {
    parseLevelTarget: function(value, fallback) {
        if (!value) return fallback || 80;
        const match = value.toString().match(/(\d+)/);
        return match ? parseInt(match[1], 10) : (fallback || 80);
    },

    parseTalentTargets: function(value) {
        return (value || "1-10-10").toString().split("-").map(part => {
            const num = parseInt(part, 10);
            if (isNaN(num)) return 1;
            return Math.max(1, Math.min(10, num));
        }).concat([1, 1, 1]).slice(0, 3);
    },

    calculateBossCount: function(currentLevel, targetLevel) {
        let total = 0;
        if (targetLevel <= currentLevel) return total;
        for (const lvl of Constants.charLevels) {
            if (lvl >= currentLevel && lvl <= targetLevel) {
                total += Constants.charBossMaterialRules[lvl] || 0;
            }
        }
        return total;
    },

    calculateTalentBookCounts: function(currentTalents, targetTalents) {
        const totals = [0, 0, 0];
        const calcOne = function(current, target) {
            const result = [0, 0, 0];
            if (current >= target || current < 1 || target > 10) return result;
        ...
        for (let i = 0; i < 3; i++) {
            const part = calcOne(currentTalents[i] || 1, targetTalents[i] || 1);
            for (let j = 0; j < 3; j++) {
                totals[j] += part[j] || 0;
            }
        }
    ...
    calculateMonsterNeedStar3: function(currentLevel, targetLevel, currentTalents, targetTalents) {
        const charBreakNeed = Utils.calcCharBreakMonster(currentLevel, targetLevel);
        const talentNeed = Utils.calcTalentMonster(currentTalents, targetTalents);
        const totalNeed = {
            star1: (charBreakNeed.star1 || 0) + (talentNeed.star1 || 0),
            star2: (charBreakNeed.star2 || 0) + (talentNeed.star2 || 0),
            star3: (charBreakNeed.star3 || 0) + (talentNeed.star3 || 0)
        };
        return Utils.convertToThreeStar(totalNeed.star1, totalNeed.star2, totalNeed.star3).totalStar3;
    },

    calculateLocalNeed: function(currentLevel, targetLevel) {
        return Utils.calcCharBreakLocal(currentLevel, targetLevel);
    }
};
