// 战斗模块
var Combat = {
    // 执行战斗并检测结束（仅内部使用，由 restoredEnergy 调用）
    _restoredEnergyAutoFightAndEndDetection: async function() {
        ...
        while (challengeTime < 5000) {
            for (let i = 1; i < 5; i++) {
                ...
                ro.dispose();
                if (res && res.text && res.text.includes("自动退出")) {
                    log.info("检测到挑战成功");
                    return;
                }
            }
    ...
    restoredEnergy: async function() {
        ...
        await this._restoredEnergyAutoFightAndEndDetection();
        await this._restoredEnergyAutoFightAndEndDetection();
        log.info("能量充满，任务结束");
        await genshin.tp(2297.6201171875, -824.5869140625);
    },
    ...
    fightBoss: async function(bossName) {
        ...
        await genshin.returnMainUi();
        // 队伍只需切换一次，避免每次循环重复切换
        if (!this._partySwitched) {
            await Utils.switchPartySafe(settings.teamName);
            this._partySwitched = true;
        }
